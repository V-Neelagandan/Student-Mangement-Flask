...
def edit_data(id):
    print(f"✅ Edit route called with id={id}")
    if request.method == "POST":
        updated_data = {
    ...
    print("Raw response JSON:", response.json())  # 👈 ADD THIS
    if response.status_code == 200:
        student_data = response.json().get("Student_data")
        print("✅ Final student_data to template:", student_data)
        print("✅ Student data from backend:", student_data)  # ← this should print!
        return render_template("Templates/edit_form.html", student=student_data, title="Edit Student")
    else:
