...
def add_data():
    if request.method == "POST":
        ...
        response = calling_student.post_student_data(student_data)
        if response.status_code == 200:
            flash('Data Added successfully', 'success')
            return redirect(url_for('Home'))
        else:
...
@app.route('/students', methods=["POST"])
def post_student():
    return calling_student.post_student_data(request.get_json())

@app.route('/students/<id>', methods=["PUT"])
def update_student(id):
    return calling_student.update_student_data(id, request.get_json())
...
def edit_data(id):
    if request.method == "POST":
        ...
        response = calling_student.update_student_data(id, updated_data)
        if response.status_code == 200:
            flash("Student data updated successfully!", "success")
            return redirect(url_for("Home"))
        else:
