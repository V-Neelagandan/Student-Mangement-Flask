...
class Student:
    ...
    #     try:
    #         quary.execute(
    #             'insert into student(Student_Name,Student_Age,Student_Contact_email,Student_Contact_Number) values(?,?,?,?)', tuple(data.values()))
    ...
    #     try:
    #         quary.execute(
    #             "Update Student Set Student_Name=? ,Student_Age =?,Student_Contact_Email=?,Student_Contact_Number =? where Id=?", tuple(data))
...
# def get_Student_data_By_id(self, id):
    ...
    def post_student_data(self,id, data):
        try:
            quary.execute(
            ...
            return make_response({"error": f"Something error occurred: {e}"}, 400)

    def update_student_data(self, id, data):
        id = int(id)
        if id <= 0:
            return make_response({"success": "Please provide a valid Id"}, 400)
        ...
        try:
            ...
            connection_string.commit()
            return make_response({"success": "Data updated successfully!"})
        except Exception as e:
            return make_response({"error": f"Something error occurred: {e}"}, 400)
